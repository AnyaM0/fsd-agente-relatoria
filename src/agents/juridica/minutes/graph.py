...
from agents.juridica.minutes.assembler import assemble_acta, write_executive_summary
from agents.juridica.minutes.assignment_planner import plan_writer_assignments
from agents.juridica.minutes.chunk_io import load_chunk_contexts_with_segmentation
...
from agents.juridica.minutes.ppt_context import convert_ppt_to_context, empty_ppt_context
from agents.juridica.minutes.review_agent import review_writer_draft, validate_final_acta
from agents.juridica.minutes.theme_discovery import discover_chunk_led_topics, discover_ppt_led_topics


class JuridicaMinutesGraphState(TypedDict, total=False):
    ppt_path: str | None
    chunk_dir: str
...
def create_juridica_minutes_graph():
    graph = StateGraph(JuridicaMinutesGraphState)
...
def run_juridica_minutes_graph(
    *,
    ...
    graph = create_juridica_minutes_graph()
    ...
    return JuridicaMinutesRunResult(
...
def load_context(state: JuridicaMinutesGraphState) -> dict[str, object]:
...
def discover_topics(state: JuridicaMinutesGraphState) -> dict[str, object]:
...
def plan_assignments_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
    assignments = plan_writer_assignments(state["themes"], state["ppt_context"], state["chunks"], state["model"], variant=state["variant"])
    if not assignments:
        raise ValueError("No writer assignments could be planned for juridica minutes.")
    return {"assignments": assignments}


def write_drafts_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
    drafts_by_assignment: dict[str, WriterDraft] = {}
    from agents.juridica.minutes.writer_agent import write_assignment_draft
...
def review_drafts_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
...
def route_after_review(state: JuridicaMinutesGraphState) -> str:
...
def revise_drafts_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
    revised = dict(state["drafts_by_assignment"])
    from agents.juridica.minutes.writer_agent import revise_assignment_draft
...
def assemble_acta_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
...
def validate_acta_node(state: JuridicaMinutesGraphState) -> dict[str, object]:
