...
def revise_assignment_draft(
    ...
    *,
    ...
    prompt = _build_prompt(assignment, chunks, variant=variant) + (
        f"\n\nCurrent section title: {current_draft.section_title}"
        f"\nCurrent draft summary: {current_draft.section_summary}"
        f"\nCurrent draft markdown:\n{current_draft.body_markdown}"
        f"\nClarification issues: {clarification_request.issues}"
        f"\nRequested changes: {clarification_request.requested_changes}"
        f"\nMust address chunk refs: {clarification_request.must_address_refs}"
    )
    draft = model.invoke_structured(
        prompt,
        MinutesDraftModel,
...
def _build_prompt(assignment: WriterAssignment, chunks: list[ChunkContext], *, variant: str) -> str:
    parts = [
        ...
        f"Tema jurídico de la reunión: {assignment.theme_title}",
...
def _writer_system_prompt(variant: str) -> str:
    if variant == "ppt_led":
        return (
            "Eres un agente redactor que redacta una sección de un acta jurídica de reunión. "
            "Captura lo discutido, las decisiones tomadas, las recomendaciones y los siguientes pasos, siempre sustentado en los chunks de transcripción. "
    ...
    return (
        "Eres un agente redactor que redacta una sección de un acta jurídica de reunión usando solo chunks de transcripción. "
        "Captura la discusión, decisiones, recomendaciones y puntos no resueltos. "
        f"{_CHRONOLOGICAL_NOTE} "
        f"{_NO_SUMMARY_NOTE} "
        f"{_FOUNDATION_NOTE} "
        "Responde en español."
