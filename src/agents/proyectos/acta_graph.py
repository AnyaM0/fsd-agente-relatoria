...
class ProyectosActaResult:
    ...
    acta_json_path: str
    proyectos_result: dict[str, Any]

    def as_dict(self) -> dict[str, Any]:
        return asdict(self)


class ProyectosActaGraphState(TypedDict, total=False):
    audio_path: str
    transcript_path: str
...
def create_proyectos_acta_graph():
    graph = StateGraph(ProyectosActaGraphState)
    ...
    graph.add_node("proyectos_minutes", create_proyectos_minutes_graph())
    graph.add_node("capture_proyectos_result", capture_proyectos_result)
    ...
    graph.add_conditional_edges(
        "prepare_run",
        route_after_prepare,
        {"transcribe_media": "transcribe_media", "segmentation": "segmentation", "proyectos_minutes": "proyectos_minutes"},
    )
    graph.add_edge("transcribe_media", "segmentation")
    graph.add_edge("segmentation", "proyectos_minutes")
    graph.add_edge("proyectos_minutes", "capture_proyectos_result")
    graph.add_edge("capture_proyectos_result", "persist_result")
    graph.add_edge("persist_result", END)
    return graph.compile()


def run_proyectos_acta_graph(
    *,
    ...
    graph = create_proyectos_acta_graph()
    ...
    final_state = graph.invoke(
        {
            ...
            "input_segmentation_result_path": None if segmentation_result_path is None else str(Path(segmentation_result_path).expanduser().resolve()),
            "variant": variant,
    ...
    return ProyectosActaResult(
        ...
        proyectos_result=final_state["proyectos_result"],
    )


def prepare_run(state: ProyectosActaGraphState) -> dict[str, object]:
...
def route_after_prepare(state: ProyectosActaGraphState) -> str:
    if state.get("audio_path"):
        return "transcribe_media"
    if state.get("transcript_path"):
        return "segmentation"
    return "proyectos_minutes"


def transcribe_media_node(state: ProyectosActaGraphState) -> dict[str, object]:
...
def run_segmentation_pipeline_node(state: ProyectosActaGraphState) -> dict[str, object]:
    from agents.shared_tools.segmentation_agent.pipeline import run_segmentation_pipeline_from_file, write_segmentation_outputs
...
def capture_proyectos_result(state: ProyectosActaGraphState) -> dict[str, object]:
    proyectos_result = {
