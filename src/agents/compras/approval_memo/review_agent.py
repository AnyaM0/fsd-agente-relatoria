...
def review_writer_draft(
    ...
    *,
    ...
    review = model.invoke_structured(
        (
            f"Revisa esta sección del memo de aprobación para la asignación {assignment.assignment_id}.\n"
        ...
        DraftReviewModel,
        system_prompt=(
            "Eres el revisor orquestador de un memo de aprobación. "
            f"{_FOUNDATION_NOTE} "
            "Aprueba solo si la sección es coherente, está alineada con el tema, está sustentada en los chunks asignados y no duplica ni contradice de forma evidente otras secciones. Responde en español."
        ),
    )
    if review.approved:
        return None
...
def validate_final_memo(markdown: str, ppt_context: PPTContext, model) -> FinalValidation:
    validation = model.invoke_structured(
        (
            "Valida este memo de aprobación como entregable final.\n"
            "Revisa consistencia, cobertura de los temas centrales de aprobación, ausencia de contradicciones y si el resumen ejecutivo coincide con el cuerpo.\n"
            f"{_FOUNDATION_NOTE}\n\n"
            f"Catálogo de diapositivas PPT:\n"
            + "\n".join(f"Slide {slide.slide_number}: {slide.title}" for slide in ppt_context.slides)
            + "\n\n"
            + f"Markdown del memo de aprobación:\n{markdown}"
        ),
        FinalValidationModel,
        system_prompt=(
            "Eres el aprobador final de un memo de aprobación. "
            f"{_FOUNDATION_NOTE} "
            "Sé estricto con secciones faltantes, contradicciones y conclusiones no soportadas. Responde en español."
        ),
    )
...
def render_draft_catalog(drafts: list[WriterDraft], *, exclude_assignment_id: str | None = None) -> str:
    ...
    for draft in drafts:
        if exclude_assignment_id is not None and draft.assignment_id == exclude_assignment_id:
            continue
        parts.append(
            f"{draft.assignment_id}: {draft.section_title}\n"
            f"Summary: {draft.section_summary}"
        )
